...
if output="$(python3 "${_export_tf_vars_script_dir}/print_tf_var_exports.py")"; then
  eval "${output}"
  echo "Exported TF_VAR_* for: deluge_web_password, home_agent_ghcr_token, home_agent_openai_api_key, grafana_admin_password, blocky_postgres_password, nextcloud_tools_app_password, alertmanager_ses_smtp_username, alertmanager_ses_smtp_password, github_runner_github_token"
else
